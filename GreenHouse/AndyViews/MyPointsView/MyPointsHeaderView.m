...
@interface MyPointsHeaderView()
@property (nonatomic, strong) UIView* lineView0;
@property (nonatomic, strong) UIView* lineView;
@property (nonatomic, strong) UIView* bgView;

@property (nonatomic, strong) UILabel* headTitleLable;
@property (nonatomic, strong) UILabel* headTitleLable1;
@end

@implementation MyPointsHeaderView
+ (instancetype)headViewWithTableView:(UITableView *)tableView{
    static NSString *headIdentifier = @"header";
    
    MyPointsHeaderView *headView = [tableView dequeueReusableCellWithIdentifier:headIdentifier];
    if (headView == nil) {
        headView = [[MyPointsHeaderView alloc] initWithReuseIdentifier:headIdentifier];
    }
...
- (void)makeUI{
    ...
    self.headTitleLable1 = [[UILabel alloc] init];
    [self.bgView addSubview:self.headTitleLable1];
    self.headTitleLable1.textColor = [MyController colorWithHexString:@"49535c"];
    self.headTitleLable1.numberOfLines = 1;
    self.headTitleLable1.textAlignment = NSTextAlignmentRight;
    self.headTitleLable1.text = @"当前总积分";
    self.headTitleLable1.font = [UIFont systemFontOfSize:16];
    
    [self.headTitleLable1 mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(0);
...
- (void)makeHeader:(MyPointsHeadModel*)model{
    self.headTitleLable.text = model.jifenStr;
}
/*
